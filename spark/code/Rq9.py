...
# 构建数据的属性schema
field1 = StructField("actor", StringType(), nullable=True)
...
loan_info_row_rdd = loan_info_row.map(lambda attributes: Row(attributes[4], float(eval(attributes[7]))))
loan_info_df = spark.createDataFrame(loan_info_row_rdd, schema)

# 数据处理
result_df = loan_info_df.groupBy("actor").sum()

# 写入数据库
result_df.write.jdbc("jdbc:mysql://master:3306/huadee?useSSL=false&useUnicode=true&characterEncoding=utf8", 'Rq9',
                     'append', conn_param)
