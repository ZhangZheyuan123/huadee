...
loan_info_row_rdd = loan_info_row.map(lambda attributes: Row(attributes[2], attributes[5], float(eval(attributes[7]))))
...
result_df = loan_info_df.groupBy("area", "type").sum()

# 写入数据库
result_df.write.jdbc("jdbc:mysql://master:3306/huadee?useSSL=false&useUnicode=true&characterEncoding=utf8", 'Rq12',
                     'append', conn_param)
