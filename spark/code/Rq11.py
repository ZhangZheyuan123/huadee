...
loan_info_row_rdd = loan_info_row.map(lambda attributes: Row(attributes[2], attributes[5]))
...
result_df = loan_info_df.groupBy("area", "type").count()

# 写入数据库
result_df.write.jdbc("jdbc:mysql://master:3306/huadee?useSSL=false&useUnicode=true&characterEncoding=utf8", 'Rq11',
                     'append', conn_param)
